...
from common.postgres_system_environment import TunerPostgresSystemEnvironment
...
class PostgresTuner():
    ...
    def __init__(self, train_queries, test_queries, experiment_config, result_dir, system_environment):
        ...
        # 2nd search space
        self.n_idxs = experiment_config['n_queries_per_workload'] * experiment_config['n_workloads_per_episode']
    ...
    def eval_best(self, config):
        ...
        # eval_time
        np.savetxt(os.path.join(self.result_dir, 'eval_time.txt'), np.asarray(eval_time), delimiter=',')
...
def main(argv):
    ...
    system_environment = TunerPostgresSystemEnvironment(tbls=experiment_config['tables'])
    ...
    train_queries = data_source.import_data(FLAGS.data_dir+'/data', label='train')
    test_queries = data_source.import_data(FLAGS.data_dir+'/data', label='test')
